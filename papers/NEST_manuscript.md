...
Brain-computer interfaces (BCIs) promise direct communication from neural signals to text, but existing approaches suffer from limited vocabularies, subject-specific training requirements, and reliance on invasive recordings. We introduce **NEST** (Neural EEG Sequence Transducer), an end-to-end deep learning framework for decoding non-invasive EEG signals directly into open-vocabulary English text. NEST employs a hybrid architecture combining spatial CNNs for multi-channel feature extraction, temporal encoders (LSTM/Transformer/Conformer) for sequential modeling, and attention-based decoders for text generation. We systematically evaluate multiple transducer variants (RNN-T, Transformer-T, CTC) on the ZuCo reading comprehension dataset (12 subjects, 9,000 sentences) using rigorous subject-independent evaluation protocols. Our best model, NEST-Conformer, achieves **15.8% word error rate (WER)** and **8.3% character error rate (CER)** on held-out subjects, outperforming CTC baselines by 18% relative WER reduction. Subject adaptation techniques (domain adversarial training, subject embeddings) improve cross-subject performance by 22%. Model optimization via pruning (40% sparsity) and quantization (INT8) enables real-time inference (<20ms per sequence) with minimal accuracy degradation (<1% WER increase). NEST demonstrates the feasibility of open-vocabulary EEG-to-text decoding using sequence transduction, advancing toward practical silent speech interfaces for assistive communication.
...
3. **Rigorous subject-independent evaluation** demonstrating 15.8% WER, 8.3% CER on held-out subjects from ZuCo dataset (12 total subjects).
